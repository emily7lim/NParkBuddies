...
Future<Album> curAlbum(username) async {
  ...
  if (response.statusCode == 200) {
    if (jsonDecode(response.body)['current_bookings'].length != 0) {
...
Future<Album> pastAlbum(username) async {
  final response = await http.get(Uri.parse(
      'https://hookworm-solid-tahr.ngrok-free.app/profiles/' +
          ...
          '/bookings'));
  if (response.statusCode == 200) {
    if (jsonDecode(response.body)['past_bookings'].length != 0) {
      return Album.fromJson(
...
class _BookingsState extends State<Bookings> {
  ...
  void initState() {
    super.initState();
    curBook = curAlbum('zhiyong78');
    pastBook = pastAlbum('nparkadmin');
  }
